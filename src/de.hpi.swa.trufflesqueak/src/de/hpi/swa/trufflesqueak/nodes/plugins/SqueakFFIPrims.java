...
public final class SqueakFFIPrims extends AbstractPrimitiveFactoryHolder {
    ...
    protected abstract static class PrimLoadSymbolFromModuleNode extends AbstractFFIPrimitiveNode implements Primitive2WithFallback {
        ...
        private static NativeObject loadSymbolFromModule(final ClassObject receiver, final String moduleSymbolName, final String moduleName,
            ...
            image.nfiSymbols.put(pointer, symbol);
        ...
        private static Source generateNFILoadSource(final SqueakImageContext image, final String moduleName) {
            final String path = getPathOrNull(image, moduleName);
            if (path != null) {
                return Source.newBuilder("nfi", String.format("load \"%s\"", path), "native").build();
            }
            return Source.newBuilder("nfi", String.format("load \"%s\"", System.mapLibraryName(moduleName)), "native").build();
        }

        @TruffleBoundary
        private static String getPathOrNull(final SqueakImageContext image, final String moduleName) {
            final String libName = System.mapLibraryName(moduleName);
            final TruffleFile libPath = image.getHomePath().resolve("lib" + File.separatorChar + libName);
            if (libPath.exists()) {
                return libPath.getAbsoluteFile().getPath();
